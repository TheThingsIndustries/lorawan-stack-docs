...
function initializeTabsAndAnchors(){var anchorTable={}
function createAnchorTable(element){const tabContainer=element.closest('.tab-content')
if(tabContainer){const tab=element.parentElement
...
function hashChanged(){const hash=window.location.hash
const id=window.location.hash.substring(1)
const key=anchorTable[id]
toggleTab(key)
document.getElementById(id).scrollIntoView()}
function addOnHashChange(){if('onhashchange'in window){window.onhashchange=function(){hashChanged(window.location.hash);}}
else{var storedHash=window.location.hash;window.setInterval(function(){if(window.location.hash!=storedHash){storedHash=window.location.hash;hashChanged(storedHash);}},1000);}}
function toggleTab(key){const activeClass='is-active'
...
function addTabs(){if(tabs.length===0)return
function handleClick(event){event.preventDefault()
let tab=event.currentTarget
let key=tab.getAttribute('data-tab')
toggleTab(key)}
if(window.sessionStorage.getItem('tabActive')){const success=toggleTab(window.sessionStorage.getItem('tabActive'))
if(success===false){let key=tabs[0].getAttribute('data-tab')
toggleTab(key)}}else if(tabs&&tabs[0]){let key=tabs[0].getAttribute('data-tab')
toggleTab(key)}
tabs.forEach(tab=>{tab.addEventListener('click',handleClick)})}
addAnchors()
addTabs()
addOnHashChange()}
function addLightboxHandler(){document.querySelectorAll('figure').forEach(fig=>{fig.addEventListener('click',event=>{basicLightbox.create(`<img src="${event.target.src}" alt="${event.target.alt}" />`).show()})})}
document.addEventListener('DOMContentLoaded',function(){addNavBarBurgers()
