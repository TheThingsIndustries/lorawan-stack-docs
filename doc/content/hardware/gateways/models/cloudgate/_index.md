...
aliases: [/gateways/cloudgate, /gateways/models/cloudgate]
image: [CloudGate.png]
---
...
This guide will help you set up the CloudGate gateway to communicate over {{% tts %}} using [LoRa Basics Station]({{< ref "/hardware/gateways/concepts/lora-basics-station" >}}).
...
On CloudGate gateways, only CUPS or LNS can be configured. Configuring CUPS will automatically configure LNS, so to use {{% tts %}}, just follow the instructions for [Connecting to CUPS]({{< ref "/hardware/gateways/concepts/lora-basics-station/cups" >}}).
