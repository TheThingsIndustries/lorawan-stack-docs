...
Authorize an application for claiming (EXPERIMENTAL)
...
```
ttn-lw-cli applications claim authorize [application-id] [flags]
```
...
```
      --api-key string          
      --api-key-expiry string   API key expiry date (YYYY-MM-DD:HH:mm) - only applicable when creating API Key
