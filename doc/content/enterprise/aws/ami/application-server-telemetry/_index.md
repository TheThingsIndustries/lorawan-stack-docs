...
aliases:
  [
    ...
    /the-things-stack/host/aws/ami/application-server-telemetry,
  ]
---
...
{{< note >}} This only works if {{% tts %}} runs in your AWS account, i.e. in a Enterprise or AWS Launcher deployment. When using the [AWS Marketplace AMI listing]({{< ref "/enterprise/aws/ami" >}}), the **AWS IoT Telemetry** option in the CloudFormation template must be set to **true** during the deployment phase. {{</ note >}}

## Setup

1. Login to the AWS Console in the _same region_ as where the CloudFormation template is deployed.
