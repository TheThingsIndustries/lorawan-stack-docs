...
{{< note >}}
These configuration methods only apply to {{% tts %}} Open Source and Enterprise deployments that follow the [Installation]({{< ref "/enterprise/docker" >}}) guide. AWS Launcher deployments can be configured using the [CloudFormation template settings]({{< ref "/the-things-stack/host/aws/ami/deployment-guide#deployment-using-aws-cloud-formation" >}}). Managed deployments such as Cloud and {{% ttss %}} are configured for you by The Things Industries.
{{</ note >}}
