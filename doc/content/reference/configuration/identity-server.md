...
- `is.read-database-uri`: Read-Only database connection URI {{< distribution-inline "Enterprise" >}}
...
- `is.oauth.ui.branding-cluster-id`: The cluster ID to show below the logo {{< distribution-inline "Enterprise" >}}
- `is.oauth.ui.branding-text`: The branding text to show below the logo {{< distribution-inline "Enterprise" >}}
...
## Tenant Administration Options {{< distribution-inline "Enterprise" >}}
...
- `is.tenancy.admin-keys`: Keys that can be used for tenant administration (16, 24 or 32 hex-encoded bytes) {{< distribution-inline "Enterprise" >}}
