...
description: ""
weight: 
distributions: ["Cloud", "AWS Launcher"]
...
{{< distributions "Cloud" "Enterprise" >}} To mark a document as applicable to only one or more distributions, do one of the following:
...
Note that if you use the `{{< distributions >}}` shortcode in a heading, Hugo will not correctly generate the ID element for it. Add the shortcode at the beginning of the paragraph following.

Available distributions are stored in `data/distributions.yml`.
